...
static QueryResult convertResult(const ::database::core::database_result& kcResult) {
    QueryResult result;
...
PreparedStatement::PreparedStatement(std::string sql) : sql_(std::move(sql)) {}

PreparedStatement& PreparedStatement::bindString(std::string_view name, std::string value) {
    params_[std::string(name)] = std::move(value);
    return *this;
}

PreparedStatement& PreparedStatement::bindInt(std::string_view name, std::int64_t value) {
    params_[std::string(name)] = value;
    return *this;
}

PreparedStatement& PreparedStatement::bindDouble(std::string_view name, double value) {
    params_[std::string(name)] = value;
    return *this;
}

PreparedStatement& PreparedStatement::bindBool(std::string_view name, bool value) {
    params_[std::string(name)] = value;
    return *this;
}
...
std::string PreparedStatement::resolve() const {
    ...
    for (const auto& [name, val] : params_) {
        ...
        while ((pos = resolved.find(placeholder, pos)) != std::string::npos) {
            ...
            if (endPos < resolved.size() &&
                (std::isalnum(resolved[endPos]) || resolved[endPos] == '_')) {
                pos = endPos;
                continue;
            }
...
GameResult<void> Transaction::commit() {
    ...
    if (!result.is_ok()) {
        return GameResult<void>::err(
            GameError(ErrorCode::TransactionFailed, "commit failed: " + result.error().message));
    }
...
GameResult<void> Transaction::rollback() {
    ...
    if (!result.is_ok()) {
        return GameResult<void>::err(
            GameError(ErrorCode::TransactionFailed, "rollback failed: " + result.error().message));
    }
    return GameResult<void>::ok();
}
...
GameResult<uint64_t> Transaction::execute(std::string_view sql) {
    ...
    if (!result.is_ok()) {
        return GameResult<uint64_t>::err(GameError(ErrorCode::QueryFailed, result.error().message));
    }
...
struct GameDatabase::Impl {
    ...
    std::size_t countActive() const {
        ...
        return static_cast<std::size_t>(std::count_if(
            pool.begin(), pool.end(), [](const PooledConnection& c) { return c.inUse; }));
    }
};
...
GameDatabase::GameDatabase() : impl_(std::make_unique<Impl>()) {}
...
GameResult<void> GameDatabase::connect(const DatabaseConfig& config) {
    if (impl_->connected.load()) {
        return GameResult<void>::err(GameError(ErrorCode::AlreadyExists, "already connected"));
    }
    ...
    for (uint32_t i = 0; i < config.minConnections; ++i) {
        ...
        if (!conn.manager) {
            ...
            disconnect();
            return GameResult<void>::err(GameError(ErrorCode::DatabaseError,
...
GameResult<QueryResult> GameDatabase::query(std::string_view sql) {
    ...
    if (!mgr) {
        return GameResult<QueryResult>::err(
            GameError(ErrorCode::ConnectionPoolExhausted, "no available connections in pool"));
    }
...
std::future<GameResult<QueryResult>> GameDatabase::queryAsync(std::string_view sql) {
...
GameResult<PreparedStatement> GameDatabase::prepare(std::string_view sql) {
    ...
    return GameResult<PreparedStatement>::ok(PreparedStatement(std::string(sql)));
}
...
GameResult<Transaction> GameDatabase::beginTransaction() {
    ...
    txnImpl->returnConnection = [implPtr](::database::database_manager* m) { implPtr->checkin(m); };

    return GameResult<Transaction>::ok(Transaction(std::move(txnImpl)));
}
