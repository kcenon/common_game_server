...
struct LobbyServer::Impl {
    ...
    explicit Impl(LobbyConfig cfg)
        : config(std::move(cfg)), queue(config.queueConfig), parties(config.maxPartySize) {}
};

// -- Construction / destruction / move ----------------------------------------

LobbyServer::LobbyServer(LobbyConfig config) : impl_(std::make_unique<Impl>(std::move(config))) {}
...
GameResult<void> LobbyServer::start() {
    if (impl_->running.load()) {
        return GameResult<void>::err(
            GameError(ErrorCode::LobbyAlreadyStarted, "lobby server is already running"));
    }
...
GameResult<void> LobbyServer::enqueuePlayer(uint64_t playerId,
                                            PlayerRating rating,
                                            GameMode mode,
                                            Region region) {
    if (!impl_->running.load()) {
        return GameResult<void>::err(
            GameError(ErrorCode::LobbyNotStarted, "lobby server is not running"));
    }
...
GameResult<PartyId> LobbyServer::createParty(uint64_t leaderId,
    ...
    auto result = impl_->parties.createParty(leaderId, leaderName, leaderRating);
...
GameResult<void> LobbyServer::disbandParty(PartyId partyId, uint64_t requesterId) {
...
GameResult<void> LobbyServer::addPartyMember(PartyId partyId,
                                             uint64_t playerId,
...
GameResult<void> LobbyServer::removePartyMember(PartyId partyId, uint64_t playerId) {
    return impl_->parties.removeMember(partyId, playerId);
}

GameResult<void> LobbyServer::promotePartyLeader(PartyId partyId,
...
std::optional<PartyId> LobbyServer::getPlayerParty(uint64_t playerId) const {
...
GameResult<void> LobbyServer::enqueueParty(PartyId partyId) {
    ...
    if (!avgRating.has_value()) {
        return GameResult<void>::err(GameError(ErrorCode::PartyNotFound, "party has no members"));
    }
...
void LobbyServer::updateRatings(PlayerRating& winnerRating, PlayerRating& loserRating) {
    float expectedWin = EloCalculator::expectedScore(winnerRating.mmr, loserRating.mmr);
    float expectedLoss = EloCalculator::expectedScore(loserRating.mmr, winnerRating.mmr);

    int32_t kWinner = EloCalculator::suggestedKFactor(winnerRating.gamesPlayed);
    int32_t kLoser = EloCalculator::suggestedKFactor(loserRating.gamesPlayed);

    winnerRating.mmr = EloCalculator::newRating(winnerRating.mmr, 1.0f, expectedWin, kWinner);
    loserRating.mmr = EloCalculator::newRating(loserRating.mmr, 0.0f, expectedLoss, kLoser);
